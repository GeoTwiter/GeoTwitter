...
class ParserREST:
    def __init__ (self, config, db_cursor):
        self.keys = config['appkeys']
        self.twitter = Twython(self.keys['app_key'],
                            self.keys['app_secret'],
                            self.keys['oauth_token'],
                            self.keys['oauth_token_secret'])
    ...
    def get_user_timeline(self, user_id, since_id):
        ...
        while process:
            ...
            if (current_time - self.time_start) > self.time_window:
            ...
            if self.timeline_request_counter == 0:
                ...
                time.sleep(self.time_window - (current_time - self.time_start))
            else:
...
class ParserStream(TwythonStreamer):
    ...
    def parse(self, username_only = True, amount_of_users = 1000):
        ...
        while self.process:
            try:
                ...
                if ((time.time() - last_error_time) > 
                            (float(self.errors['connection']['timeout']) * 
                            float(self.errors['connection']['attempts']))):
    ...
    def on_success(self, data):
        if 'user' in data:
            ...
            if self.amount_of_users > 1:
                self.amount_of_users = self.amount_of_users - 1
            else:
                self.disconnect()
                self.process = False
